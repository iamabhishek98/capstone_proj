...
ACTIONS = ["dab","elbowkick","gun","hair","listen","pointhigh","sidepump","wipetable"]
POSITIONS = ['1 2 3', '3 2 1', '2 3 1', '3 1 2', '1 3 2', '2 1 3']
LOG_DIR = os.path.join(os.path.dirname(__file__), 'evaluation_logs')
NUM_MOVE_PER_ACTION = 2
TOTAL_MOVES = NUM_MOVE_PER_ACTION * len(ACTIONS)
N_TRANSITIONS = 6
...
class Server(threading.Thread):
            ...
            if data:
                try:
                    ...
                    if decrypted_message['action'] == "logout":
                        ...
                        predicted_position, predicted_action, sync = decrypted_message['position'],decrypted_message['action'],decrypted_message['sync']
                        data = dict()
                        data['Pp'] = predicted_position
                        data['Pa'] = predicted_action
                        data['Rp'] = pos_string  
                        data['Rm'] = self.action
                        ...
                        print("Position: {} | action: {} | Sync: {}".format(decrypted_message['position'],
                                                                  decrypted_message['action'], 
                                                                  decrypted_message['sync']))
            # data['is_action_correct'] = (self.action == predicted_action)
            # data['is_position_correct'] = (pos_string == predicted_position) 
                        print("Position: {} | action: {} \n\n  =====================================".format(decrypted_message['position'] == self.dancer_positions,
                                                                  decrypted_message['action'] == self.action))

                        if len(finaldata)>TOTAL_MOVES-1:
                            for i in finaldata:
                                s = "Pred:{} Real:{}\t| Pred:{} Real:{} \nMove:{} | Pos:{} | Sync:{} Res:{}\n======================= ".format(
                                    i['Pp'],
                                    i['Rp'],
                                    i['Pa'],
                                    i['Rm'],
                                    i['is_action_correct'],
                                    i['is_position_correct'],
                                    i['sync'],
                                    data['response_time']
                                )
                                print(s)
    ...
    def set_next_action(self):
        ...
        print("NEXT MOVE {} | POS {}".format(self.action,self.dancer_positions))
        print("Result: ")
        self.timer = threading.Timer(self.timeout, self.set_next_action)
