...
class Server(threading.Thread):
    ...
    def run(self):
        while not self.shutdown.is_set():
            ...
            # data['is_position_correct'] = (pos_string == predicted_position) 
                        ...
                        self.results.append((self.idx,decrypted_message['position'] == self.dancer_positions,decrypted_message['action'] == self.action))      
    ...
    def send_dancer_positions(self):
        ...
        print('Set Dancers to Positions: {}'.format(dancer_positions))
    ...
    def set_next_action(self):
        ...
        if self.idx < self.n_moves:
            ...
            for i in range(len(self.results)):
                print("{}. Pos {} | Move {}".format(self.results[i][0], self.results[i][1],self.results[i][2]))
                self.logout = True
                self.stop()
                sys.exit(1)
